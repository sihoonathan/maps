...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        restaurants_cluster = group_by_centroid(restaurants, old_centroids)
        centroids = [find_centroid(restaurant_cluster) for restaurant_cluster in restaurants_cluster]
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    user_not_reviewed = [[restaurant_name(restaurant), predictor( restaurant)] for restaurant in restaurants if restaurant_name(restaurant) not in [restaurant_name(review) for review in reviewed]]

    return {name: rating for name, rating in user_reviewed + user_not_reviewed}
    # END Question 9
...
user = make_user('Cheapskate', [make_review('A', 2), make_review('B', 5),
make_review('C', 2), make_review('D', 5),])
